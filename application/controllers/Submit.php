...
class Submit extends CI_Controller
{
	...
	public function __construct()
	{
		...
		$this->problems_root = $this->settings_model->get_setting('problems_root');
		//$this->problems = $this->problems_model->all_problems();
        /*
	...
	public function _check_language($str)
	{
		if ($str=='0')
			return FALSE;
		if (in_array( strtolower($str),array('c', 'c++', 'python 2', 'python 3', 'java', 'zip', 'pdf', 'free pascal')))
			return TRUE;
		return FALSE;
	}
    ...
    public function online()
	{
		...
		$this->twig->display('pages/submitonline.twig', $this->data);
	...
	private function _upload()
	{
		...
		//		break;
		//	}
        $this->problem = $this->problem_model->get_problem($this->input->post('problem'));
		...
		if ( ! preg_match('/^[a-zA-Z0-9_\-()]+$/', $this->file_name) )
			show_error('文件名只能为数字及字母的组合');

		$user_dir = rtrim($this->problems_root, '/').'/p'.$this->problem['id'].'/'.$this->user->username;
		if ( ! file_exists($user_dir))
			mkdir($user_dir, 0700);
		$config['upload_path'] = $user_dir;
		$config['allowed_types'] = '*';
		$config['max_size']	= $this->settings_model->get_setting('file_size_limit');
		$config['file_name'] = $this->file_name."-".($this->user->total_submits+1).".".$this->ext;
		...
		if ($this->upload->do_upload('userfile'))
		{
            ...
            //return $this->db->get_where('problems', array('id'=> $this->problem['id']))->result_array()[0];
            $this->db->where('id',$this->problem['id'])->update('problems',array('total_submits'=>($this->problem['total_submits']+1)));
			return TRUE;
		}
